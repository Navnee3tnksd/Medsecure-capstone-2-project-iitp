# Medsecure-capstone-2-project-iitpMedSecure
...
Bun 1.3+
PostgreSQL database
Supabase project with a storage bucket named medical-reports
Setup
cd monorepo
...
User flow
Sign up at /signup → redirected to dashboard.
Profile — add blood type, allergies, emergency contact (shown on QR view).
Health logs — /records for daily vitals.
Reports — upload from dashboard.
QR — /qr shows code linking to http://localhost:3001/view/{token}.
Scan — responder sees read-only emergency dashboard (no account needed).
Security notes
